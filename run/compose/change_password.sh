...
run(){
    sudo docker exec try-iris bash -c 'echo '"$1"' >/tmp/pwd.txt' > /dev/null 2>&1
    sudo docker exec try-iris bash -c "/usr/irissys/dev/Cloud/ICM/changePassword.sh /tmp/pwd.txt" > /dev/null 2>&1
    sudo docker exec try-iris bash -c "iris start iris" > /dev/null 2>&1
